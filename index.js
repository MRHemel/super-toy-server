...
async function run() {
    try {
        // Connect the the client to the server	(optional starting in v4.7)
        await client.connect();

        const toyCollection = client.db("toyStore").collection("toys")
        const userCollection = client.db("toyStore").collection("mytoy")
...
app.listen(port, () => {
    console.log(`toy store is running on ${port}`)
})